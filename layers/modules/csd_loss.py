...
class CSDLoss(nn.Module):
    ...
    def forward(self, args,conf, conf_flip, loc, loc_flip, conf_consistency_criterion,yolo=False,num_classes = 5):
        ...
        if yolo:
            ...
            background_score = conf[:, :, -1].clone()
            each_val, each_index = torch.max(conf_class, dim=2)
            mask_val = background_score > 0.5
            mask_val = mask_val.data
            
            # remove confidence 
            conf = conf[:, :, :-1].clone()
            conf_flip = conf_flip[:,:,:-1].clone()
        else:
            conf_class = conf[:, :, 1:].clone()
        ...
        conf_sampled = conf[mask_conf_index].view(-1, num_classes).clone()
        conf_sampled = conf[mask_conf_index].view(-1, num_classes).clone()
        
        loc_sampled = loc[mask_loc_index].view(-1, 4).clone()


        conf_sampled_flip = conf_flip[mask_conf_index].view(-1, num_classes).clone()
