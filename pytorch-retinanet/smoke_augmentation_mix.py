        ...
        if self.search_smoke(target) is not True:
            ...
            if x > threshold:
                ...
                try:
                    ...
                    new_label = np.array([min_x ,min_y ,max_x ,max_y ,SMOKE_ID])
                    new_label = new_label.reshape((1,5))
                    target = np.append(target ,new_label , axis = 0 )
                    img = img.astype(np.float32)/255.0
                except:
                    return {'img':ori_img , 'annot':ori_target}
                # target.append(new_label)
        return {'img':img , 'annot':target}
