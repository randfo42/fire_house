    ...
    def parse(path):
        ...
        try:
            ...
            for filename in filenames:
                ...
                xml = open(os.path.join(dir_path, filename), 'rt', encoding='UTF8')
                try:
                    tree = Et.parse(xml)
                except:
                ...
                data[root.find("filename").text[:-4]] = annotation

                printProgressBar(progress_cnt + 1, progress_length,
                                 prefix='VOC Parsing:'.ljust(15), suffix='Complete', length=40)
                progress_cnt += 1

            return True, data, error_list
